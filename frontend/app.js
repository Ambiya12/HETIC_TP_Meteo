...
function updateMetrics(data) {
  ...
  const tempValue = data.temperature !== undefined ? data.temperature.toFixed(1) : '--';
  ...
  const humidityValue = data.humidity !== undefined ? data.humidity.toFixed(1) : '--';
  ...
  state.currentUnit = data.unit || 'celsius';
  updateModeIndicator(state.currentUnit);
  
  if (data.mode) {
    elements.dataMode.textContent = data.mode === 'simulation' ? 'SIMULATION' : 'RÉEL';
  }
...
function renderHistory() {
  if (state.history.length === 0) {
    elements.history.innerHTML = '<div class="history-empty">En attente de données...</div>';
    return;
  }
...
if (window.location.hostname === 'localhost') {
  console.log('%c Mode Debug', 'color: #00d9ff; font-weight: bold; font-size: 14px;');
